...
def process_message(message_str, element_time, csv_file):
    ...
    if message_str:
        ...
        if ord(message_str[0]) == 0x1b:
            ...
            for md_message_type, md_message_header in md_messages.items():
                ...
                if md_message.startswith(md_message_header):
                    ...
                    line = ["MD Logger", time, md_message_type, logger_msg, hex(0), element_time, ""]
        ...
        if not found_md_message:
            ...
            line = ["FRC", "", "", "", hex(0), element_time, message_str]
            csv_file.writerow(line)
...
def process_tagversion_record(tagversion_record, csv_file):
    ...
    while tagversionElement_start >= 0:
        ...
        if next_tag == "message":
            ...
            is_md_message = process_message(message_content, time_content, csv_file)
            if is_md_message:
                pass
            else:
                pass
...
def process_logfile(logfile_directory, logfile_name):
            ...
            if message.startswith(record_type):
                if action != "skip":
                    if record_type == "<TagVersion>":
                        process_tagversion_record(message, csv_file)
    ...
    print(f"\nMD messages found in the log file:")
...
def main():
    ...
    options = {"logfile_directory" : "C:\\Temp",
